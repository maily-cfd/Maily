...
export function CommandCenter({ userName, onOpenExistingDraft }: {
  ...
  // Refresh EVERYTHING on the feed at once — today's snapshot, the cross-app
  // world synthesis, and the analytics/recommendations — bypassing every cache.
  // Drives the header refresh button; its own spinner is independent of the
  // per-section retries so the whole dashboard reloads in one action.
  const [refreshingAll, setRefreshingAll] = useState(false);
  const refreshAll = useCallback(async () => {
    if (refreshingAll) return;
    setRefreshingAll(true);
    try {
      await Promise.allSettled([refetchToday(), refetchWorld(), refreshAnalytics()]);
    } finally {
      setRefreshingAll(false);
    }
  }, [refreshingAll, refetchToday, refetchWorld, refreshAnalytics]);
  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-4 sm:px-6 py-6 space-y-8">
      ...
      <motion.section initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            ...
            title="Refresh the whole dashboard — today, your world, and analytics"
            className="shrink-0 inline-flex items-center gap-1.5 h-8 px-3 rounded-[18px] border border-[#e5e5e5] bg-white text-[12.5px] font-medium text-[#171717] hover:bg-[#f5f5f5] transition-colors disabled:opacity-60 disabled:cursor-default"
          >
            ...
            {hasReconnectIssue
              ...
              : sift?.headline
                ...
                : recsError
                  ...
                  ? <span className="text-[#e7000b]">AI error — {recsError}</span>
                  : today?.briefing?.trim()
                    || (nothingPressing
      ...
      {needsIntegrationsNudge && (
        <div className="mono-card rounded-3xl p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="h-9 px-3.5 rounded-[18px] border border-[#e5e5e5] bg-white text-[12.5px] font-medium text-[#737373] hover:text-[#0a0a0a] hover:bg-[#f5f5f5] transition-colors"
            >
              Not now
            </button>
            <button
              ...
              className="h-9 px-3.5 rounded-[18px] bg-[#0a0a0a] text-[#fafafa] text-[12.5px] font-medium hover:bg-[#171717] transition-colors inline-flex items-center gap-1.5"
            >
      ...
      {connectedApps
        ...
        : <div className="mono-card rounded-3xl h-[104px] animate-pulse" />}
      ...
      {worldEntities.length > 0 ? (
        ...
        <Section title="Your world right now" sub="Reading across your connected apps…">
          <div className="grid sm:grid-cols-2 gap-2.5">
            {[0, 1, 2, 3].map(i => <div key={i} className="h-32 rounded-3xl bg-[#ececec] animate-pulse" />)}
          </div>
        </Section>
      ) : world?.error ? (
...
function FeedErrorCard({ message, onRetry }: { message: string; onRetry?: () => void }) {
  return (
    <div className="rounded-3xl mono-card p-5 flex items-start gap-3" style={{ borderColor: 'rgba(231,0,11,0.3)' }}>
      <AlertTriangle className="w-4 h-4 text-[#e7000b] shrink-0 mt-0.5" />
      <div className="min-w-0 flex-1">
        <p className="text-[13px] font-semibold text-[#e7000b]">AI error</p>
        <p className="text-[12.5px] text-arcus-fg-secondary mt-0.5 break-words leading-relaxed">{message}</p>
      </div>
      {onRetry && (
        <button
          onClick={onRetry}
          className="shrink-0 inline-flex items-center gap-1 h-7 px-2.5 rounded-[18px] border border-[#e5e5e5] bg-white text-[12px] font-medium text-[#171717] hover:bg-[#f5f5f5] transition-colors"
        >
...
function StatTile({ icon, value, label, tone }: { icon: React.ReactNode; value: number; label: string; tone: 'attn' | 'calm' | 'good' }) {
  return (
    <div className="rounded-3xl mono-card px-5 py-4">
      <div className={cn(
        'inline-flex items-center justify-center w-8 h-8 rounded-[10px] mb-2.5 bg-[#f5f5f5]',
        tone === 'calm' ? 'text-[#737373]' : 'text-[#0a0a0a]',
      )}>
        {icon}
      </div>
      <div className="text-[30px] font-semibold tracking-[-0.02em] text-arcus-fg tabular-nums leading-none">{value}</div>
      <div className="text-[11px] font-medium uppercase tracking-[0.04em] text-arcus-fg-tertiary mt-1.5">{label}</div>
    </div>
  );
}
...
// Monochrome status treatment — the icon + label carry the meaning; the badge is
// one neutral soft chip (no color) per the clinical light system. `awaiting_you`
// (the one that's on YOU) gets the solid ink chip so it still reads as the ask.
const STATUS_META: Record<ConvoStatus, { label: string; cls: string; fill: string; Icon: any }> = {
  awaiting_you: { label: 'Awaiting your reply', cls: 'bg-[#0a0a0a] text-[#fafafa] border-transparent', fill: 'bg-[#0a0a0a]', Icon: Reply },
  waiting_on_them: { label: 'Waiting on them', cls: 'bg-[#f5f5f5] text-[#525252] border-[#e5e5e5]', fill: 'bg-[#a3a3a3]', Icon: Clock },
  meeting_booked: { label: 'Meeting booked', cls: 'bg-[#f5f5f5] text-[#171717] border-[#e5e5e5]', fill: 'bg-[#737373]', Icon: Calendar },
  active: { label: 'Active thread', cls: 'bg-[#f5f5f5] text-[#525252] border-[#e5e5e5]', fill: 'bg-[#737373]', Icon: MessageSquare },
};
...
// Monochrome kind badge — an outline chip; the label alone tells you what they
// are. No per-kind color (the system keeps a single hue, reserved for Ember).
const KIND_META: Record<RelationshipKind, { label: string; cls: string }> = {
  investor:  { label: 'Investor',  cls: 'bg-transparent text-[#404040] border-[#d4d4d4]' },
  candidate: { label: 'Candidate', cls: 'bg-transparent text-[#404040] border-[#d4d4d4]' },
  customer:  { label: 'Customer',  cls: 'bg-transparent text-[#404040] border-[#d4d4d4]' },
  lead:      { label: 'Lead',      cls: 'bg-transparent text-[#404040] border-[#d4d4d4]' },
  vendor:    { label: 'Vendor',    cls: 'bg-transparent text-[#404040] border-[#d4d4d4]' },
  press:     { label: 'Press',     cls: 'bg-transparent text-[#404040] border-[#d4d4d4]' },
};
...
function ReceiptQuote({ text }: { text: string }) {
  return (
    <div className="mt-2.5 flex gap-2 rounded-[10px] mono-inset px-2.5 py-2">
      <Quote className="w-3.5 h-3.5 text-arcus-fg-muted shrink-0 mt-[1px]" />
      <p className="text-[12px] text-arcus-fg-secondary leading-relaxed line-clamp-2">{text}</p>
    </div>
  );
}
      ...
      <div className="mono-card rounded-3xl p-5">
        <div className="flex items-center justify-between gap-3 mb-3.5">
          <div className="flex items-center gap-2.5 min-w-0">
            <span className="inline-flex items-center justify-center w-7 h-7 rounded-[10px] bg-[#f5f5f5] text-[#737373] shrink-0">
          ...
          {apps.map(app => {
            ...
            return (
              <button
                ...
                className={cn(
                  'group inline-flex items-center gap-2 h-9 pl-2.5 pr-2.5 rounded-[18px] border transition-all duration-200',
                  on
                    ? 'border-[#e5e5e5] bg-[#fafafa]'
                    : 'border-[#e5e5e5] hover:bg-[#f5f5f5] hover:border-[#d4d4d4]',
                )}
              >
                ...
                <span className={cn('text-[12.5px] font-medium', on ? 'text-[#0a0a0a]' : 'text-[#737373]')}>{app.name}</span>
                {on
                  ? <Check className="w-3.5 h-3.5 text-[#0a0a0a] shrink-0" strokeWidth={2.5} />
                  : <Plus className="w-3.5 h-3.5 text-[#a3a3a3] shrink-0 group-hover:text-[#737373]" />}
              </button>
            );
          })}
        </div>
...
function AppChip({ chip }: { chip: WorldAppChip }) {
  ...
  return (
    <span
      title={chip.evidence}
      className="inline-flex items-center gap-1 text-[10.5px] px-2 py-0.5 rounded-[18px] bg-[#f5f5f5] border border-[#e5e5e5] text-[#525252]"
    >
      <Icon className="w-3 h-3 shrink-0 text-[#737373]" />
      {chip.label}
    </span>
  );
}
...
function WorldCard({ e, onHandle }: { e: WorldEntry; onHandle: () => void }) {
  ...
  return (
    <button
      onClick={onHandle}
      className={cn(
        'group text-left rounded-3xl mono-card mono-card-hover p-5',
        e.atRisk && 'ring-1 ring-[#e7000b]/35',
      )}
    >
      <div className="flex items-center justify-between gap-2 mb-2">
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          {kind && (
            <span className={cn('shrink-0 px-2 py-0.5 rounded-[18px] border text-[9.5px] font-semibold uppercase tracking-wide', kind.cls)}>
              {kind.label}
            </span>
          )}
        </div>
        {e.atRisk ? (
          <span className="shrink-0 inline-flex items-center gap-1 px-2 py-0.5 rounded-[18px] border text-[10.5px] font-medium bg-[#e7000b]/[0.08] text-[#e7000b] border-[#e7000b]/25">
...
function SlippingRow({ e, onHandle }: { e: WorldEntry; onHandle: () => void }) {
  ...
  const receipt = e.receipts?.[0];
  return (
    <div className="rounded-3xl mono-card ring-1 ring-[#e7000b]/25 p-5 flex items-start gap-3">
      <div className="w-9 h-9 rounded-[10px] bg-[#e7000b]/[0.08] flex items-center justify-center shrink-0 text-[#e7000b]">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="text-[11px] text-[#e7000b] font-medium shrink-0">at risk</span>
        </div>
        ...
        <div className="flex items-center gap-2 mt-2.5">
          <button onClick={onHandle} className="inline-flex items-center gap-1.5 h-8 px-3.5 rounded-[18px] bg-[#0a0a0a] text-[#fafafa] text-[12.5px] font-medium hover:bg-[#171717] transition-colors">
...
function ReplyRow({ who, subject, reason, when, signals, onDraft, onOpen }: { who: string; subject: string; reason: string; when: string; signals?: string[]; onDraft: () => void; onOpen: () => void }) {
  return (
    <div className="rounded-3xl mono-card p-5 flex items-start gap-3">
      <div className="w-9 h-9 rounded-[10px] bg-[#f5f5f5] flex items-center justify-center shrink-0 text-[#737373]">
        ...
        {signals && signals.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-2">
            {signals.slice(0, 3).map((s, i) => (
              <span key={i} className="text-[10.5px] px-2 py-0.5 rounded-[18px] bg-[#f5f5f5] text-[#525252] border border-[#e5e5e5]">{s}</span>
            ))}
          </div>
        )}
        <div className="flex items-center gap-2 mt-2.5">
          <button onClick={onDraft} className="inline-flex items-center gap-1.5 h-8 px-3.5 rounded-[18px] bg-arcus-fg text-arcus-fg-inverse text-[12.5px] font-medium hover:opacity-90 transition-opacity">
            <Reply className="w-3.5 h-3.5" /> Draft reply
          </button>
          <button onClick={onOpen} className="inline-flex items-center gap-1.5 h-8 px-3.5 rounded-[18px] border border-arcus-border text-arcus-fg-secondary text-[12.5px] font-medium hover:bg-arcus-surface-hover transition-colors">
            Open thread
          </button>
        </div>
      </div>
...
function MeetingRow({ title, start, attendeeCount, isExternal, meetLink, onPrep }: { title: string; start: string; attendeeCount: number; isExternal: boolean; meetLink: string | null; onPrep: () => void }) {
  return (
    <div className="rounded-3xl mono-card p-5 flex items-center gap-3">
      ...
      <div className="flex items-center gap-2 shrink-0">
        {meetLink && (
          <a href={meetLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 h-8 px-3.5 rounded-[18px] border border-arcus-border text-arcus-fg-secondary text-[12.5px] font-medium hover:bg-arcus-surface-hover transition-colors">
            Join
          </a>
        )}
        <button onClick={onPrep} className="inline-flex items-center gap-1.5 h-8 px-3.5 rounded-[18px] bg-arcus-fg text-arcus-fg-inverse text-[12.5px] font-medium hover:opacity-90 transition-opacity">
...
function CommitmentRow({ item, onDo }: { item: any; onDo: () => void }) {
  ...
  return (
    <div className={cn('rounded-3xl mono-card p-5 flex items-start gap-3', overdue && 'ring-1 ring-[#e7000b]/25')}>
      <div className={cn('w-9 h-9 rounded-[10px] flex items-center justify-center shrink-0', overdue ? 'bg-[#e7000b]/[0.08] text-[#e7000b]' : 'bg-[#f5f5f5] text-[#737373]')}>
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-2 mt-1 flex-wrap">
          ...
          {overdue
            ? <span className="text-[11px] font-semibold text-[#e7000b]">overdue</span>
            : dueLabel && <span className="text-[11.5px] text-arcus-fg-muted">due {dueLabel}</span>}
        </div>
      </div>
      <button onClick={onDo} className="shrink-0 inline-flex items-center gap-1.5 h-8 px-3.5 rounded-[18px] border border-arcus-border text-arcus-fg-secondary text-[12.5px] font-medium hover:bg-arcus-surface-hover transition-colors">
...
function RecCard({ r, onDo }: { r: Rec; onDo: () => void }) {
  return (
    <button onClick={onDo} className="group text-left rounded-3xl mono-card mono-card-hover p-5">
      <div className="flex items-start justify-between gap-2 mb-1.5">
        <span className="text-[14px] font-semibold text-arcus-fg leading-snug">{r.title}</span>
        {r.atRisk && (
          <span className="shrink-0 inline-flex items-center gap-1 px-2 py-0.5 rounded-[18px] bg-[#e7000b]/[0.08] text-[#e7000b] border border-[#e7000b]/25 text-[10.5px] font-medium">
            <AlertTriangle className="w-3 h-3" /> at risk
          </span>
        )}
      </div>
...
function AgentRunRow({ run }: { run: AgentRunItem }) {
  ...
  return (
    <div className="rounded-3xl mono-card p-5 flex items-start gap-3">
      <div className={cn('w-9 h-9 rounded-[10px] flex items-center justify-center shrink-0',
        run.status === 'error' || run.status === 'transient_error' ? 'bg-[#e7000b]/[0.08] text-[#e7000b]' : 'bg-[#f5f5f5] text-[#0a0a0a]')}>
        ...
        {chips.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-2">
            {chips.map(({ k, n }) => {
              const I = APP_ICON[k];
              return (
                <span key={k} className="inline-flex items-center gap-1 text-[10.5px] px-2 py-0.5 rounded-[18px] bg-[#f5f5f5] text-[#525252] border border-[#e5e5e5] capitalize">
...
// The frosted dot-stipple + refraction sheen belonged to the glass system. The
// clinical monochrome reference is flat — every surface is a solid tone, no
// gradients or textures — so this renders nothing now (kept as a no-op so its
...
function StatPill({ icon, value, label, accent }: {
  ...
  return (
    <div className="mono-card mono-card-hover rounded-3xl p-4 relative overflow-hidden">
      <GlassTexture />
      <div className="relative">
        <span
          className="inline-flex items-center justify-center w-8 h-8 rounded-[10px] bg-[#f5f5f5]"
...
function WeekAreaChart({ week, onRefresh, refreshing }: { week: WeekData; onRefresh: () => void; refreshing: boolean }) {
  ...
  return (
    <div className="mono-card rounded-3xl p-5 relative overflow-hidden">
      <GlassTexture />
      <div className="relative flex items-start justify-between mb-1">
        ...
        <button
          ...
          className="w-8 h-8 rounded-[10px] flex items-center justify-center text-arcus-fg-tertiary hover:text-arcus-fg hover:bg-[#f5f5f5] transition-colors shrink-0"
        >
          ...
          <AreaChart data={week.days} margin={{ top: 14, right: 8, bottom: 0, left: 8 }}>
            ...
            <RTooltip
              ...
              content={({ active, payload }) => {
                ...
                return (
                  <div className="mono-card rounded-[10px] px-2.5 py-1.5">
...
function AnalyticsTileSkeleton({ h }: { h: string }) {
  return <div className={cn('bg-[#ececec] rounded-3xl animate-pulse', h)} />;
}
  ...
  if (!week || !week.hasData) {
    return (
      <Section title="Your week" sub="Arcus activity, last 7 days">
        <div className="mono-card rounded-3xl p-8 text-center relative overflow-hidden">
          <GlassTexture />
          <div className="relative">
            <div className="w-11 h-11 rounded-[18px] bg-[#f5f5f5] flex items-center justify-center mx-auto mb-3 text-arcus-fg-tertiary">
    ...
    <div className="pt-1 border-t border-arcus-border/60">
      ...
      >
        <span className="inline-flex items-center gap-2 text-[12.5px] text-arcus-fg-tertiary group-hover:text-arcus-fg-secondary transition-colors">
          <CheckCircle2 className="w-3.5 h-3.5 text-[#737373] shrink-0" />
    ...
    <div className="mx-auto w-full max-w-5xl px-4 sm:px-6 py-6 space-y-8 animate-pulse">
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 pt-2">
          {[0, 1, 2, 3].map(i => <div key={i} className="h-24 bg-[#ececec] rounded-3xl" />)}
        </div>
      </div>
      <div className="h-40 bg-[#ececec] rounded-3xl" />
      <div className="grid sm:grid-cols-2 gap-2.5">
        {[0, 1, 2, 3].map(i => <div key={i} className="h-28 bg-[#ececec] rounded-3xl" />)}
      </div>
    </div>
  );
}
