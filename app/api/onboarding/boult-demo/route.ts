/**
 * POST /api/onboarding/boult-demo
 *
...
export async function POST(request: NextRequest) {
  ...
  // with that flag already true, so "Watch Boult work" 403'd. Only paid users
  // should be steered to the real (paywalled) Boult path.
  let isPaid = false;
  try {
  ...
  if (isPaid) {
    return new Response(
      JSON.stringify({
        error: 'paid_user',
        message: 'You’re already on a plan — open Boult from the app instead.',
      }),
      ...
      try {
        ...
        for (const id of messageIds.slice(0, Math.min(8, count * 3))) {
          try {
            ...
            logEvent({ channel: 'failures', event: 'onboarding_boult_demo_read', description: String(e?.message || e) });
          }
        }
        ...
        for (let i = 0; i < targets.length; i++) {
          ...
          try {
            ...
            logEvent({ channel: 'failures', event: 'onboarding_boult_demo_ai', description: String(e?.message || e) });
          ...
          try {
            ...
            logEvent({ channel: 'failures', event: 'onboarding_boult_demo_draft', description: String(e?.message || e) });
            emit('tool_result', {
              tool: 'gmail_draft_reply',
              success: false,
        ...
        logEvent({ channel: 'failures', event: 'onboarding_boult_demo', description: String(e?.message || e) });
